...
function toRow(item) {
  ...
  return {
    ...
    price: item.price,
    requires_bagel_type: !!item.requiresBagelType,
    excludes_side: !!item.excludesSide,
...
module.exports = {
  async up(queryInterface) {
    const [tableRows] = await queryInterface.sequelize
      .query("SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'nursing_home_menu_items'")
      .catch(() => [[]]);
    if (!Array.isArray(tableRows) || tableRows.length === 0) return;

    const [countRows] = await queryInterface.sequelize.query(
      "SELECT COUNT(*)::int AS count FROM nursing_home_menu_items"
    ).catch(() => [[]]);

    const countVal = countRows && countRows[0] ? Number(countRows[0].count) : 0;
    if (countVal > 0) return;

    const rows = MENU_ITEMS_RAW.map(toRow);
    await queryInterface.bulkInsert('nursing_home_menu_items', rows, {});
  },
  ...
  async down(queryInterface) {
    ...
    if (Array.isArray(tableRows) && tableRows.length > 0) {
      await queryInterface.bulkDelete('nursing_home_menu_items', null, {});
    }
  }
};
