...
const Icons = {
  ...
  orders: (
    <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
      <path d="M14 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V8l-6-6zm4 18H6V4h7v5h5v11zM8 12h8v2H8v-2zm0 4h8v2H8v-2zm0-8h3v2H8V8z" />
    </svg>
  ),
  user: (
      ...
      >
        ...
        <nav className="admin-sidebar__nav">
          {navItems.map((item) => {
            ...
            const isActive =
              item.id === 'dashboard'
                ? path === item.path || path.endsWith(`/${facilitySlug}`) || path.endsWith(`/${facilitySlug}/`)
                : item.id === 'menu'
                  ? path === item.path || path.includes(`/${facilitySlug}/menu`)
                  : item.id === 'orders'
                    ? path.includes(`/${facilitySlug}/orders`) || path.includes(`/${facilitySlug}/order/`)
                    : path === item.path;
            return (
              <button
                key={item.id}
